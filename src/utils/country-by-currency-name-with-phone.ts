import { CountryByCurrencyWithPhoneName } from "@/types/pages/sign-in";

const countryByCurrencyNames: CountryByCurrencyWithPhoneName[] = [
  {
    name: "Bahamas",
...
const sortCountryByCurrencyNames = (sort: string = "asc") => {
  return countryByCurrencyNames.sort((a, b) => {
    if (sort === "asc") {
...
const getCountryByCurrencyNamesBySearch = (name: string = "") => {
  const _name = name?.toLowerCase() ?? "";
  return name.length
    ? sortCountryByCurrencyNames().filter(
        (country) =>
          country.name.toLowerCase().includes(_name) ||
          country.currency_code.toLowerCase().includes(_name) ||
          country.currency_name.toLowerCase().includes(_name),
      )
    : sortCountryByCurrencyNames();
};

const getAllAlpha_2 = () => {
  return sortCountryByCurrencyNames().map((country) =>
    country.alpha_2.toLowerCase(),
  );
};

export {
  countryByCurrencyNames,
  getAllAlpha_2,
  getCountryByCurrencyNamesBySearch,
};
